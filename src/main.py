...
shaderProgram_skybox = shaderLoaderV3.ShaderProgram(
    "shaders/vert_skybox.glsl", "shaders/frag_skybox.glsl")
...
normal_loc = 2
glBindAttribLocation(shaderProgram_obj.shader, normal_loc, "color")
glVertexAttribPointer(normal_loc, obj.size_normal, GL_FLOAT,
                      GL_FALSE, obj.stride, ctypes.c_void_p(obj.offset_normal))
glEnableVertexAttribArray(normal_loc)

img_data, img_width, img_height = load_image(
    "objects/stormtrooper/stormtrooper.jpg", flip=True)

# Create a texture object
texture_id = glGenTextures(1)
# Bind the texture object. That is, make it the active one.
...
# This is optional since we are using only one texture unit. It is attached to texture unit 0 (GL_TEXTURE0) by default,
# but a wise man once said, "Explicit is better than implicit."
# Okay this might be confusing. Here 0 indicates texture unit 0. Note that "cubeMapTex" is a sampler variable in the fragment shader. It is not an integer.
...
# Create a slider for the rotation
